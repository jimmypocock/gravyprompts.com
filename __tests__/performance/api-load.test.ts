...
describe("API Load Tests", () => {
  ...
  async function runLoadTest(
    ...
    mockFetch.mockImplementation(async () => {
      ...
      return {
        ...
        json: async () =>
          success ? { data: "mock" } : { error: "Server error" },
      } as Response;
    });
    ...
    for (let user = 0; user < config.concurrentUsers; user++) {
      const userPromise = (async () => {
        // Ramp up delay
        const rampUpDelay =
          (user / config.concurrentUsers) * config.rampUpTime * 1000;
  ...
  describe("Template List API Load Tests", () => {
    ...
    it("should handle search query load", async () => {
      ...
      const result = await runLoadTest(
        "/templates?search=email&limit=10",
        config,
      );
  ...
  describe("Performance Regression Tests", () => {
    ...
    it("should maintain baseline performance for search queries", async () => {
      ...
      const result = await runLoadTest(
        "/templates?search=marketing+email&limit=20",
        config,
      );
