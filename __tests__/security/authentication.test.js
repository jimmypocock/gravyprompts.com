...
describe("Authentication Security Tests", () => {
  ...
  describe("Rate Limiting Protection", () => {
    ...
    it("should reset rate limits after successful authentication", async () => {
      ...
      const successResponse = await simulateLoginAttempt(
        email,
        "correct-password",
      );
  ...
  describe("Password Security", () => {
    it("should enforce password complexity requirements", async () => {
      ...
      for (const password of weakPasswords) {
        const response = await simulatePasswordChange(
          "user@example.com",
          password,
        );
...
async function simulateProtectedEndpoint(event) {
  ...
  const authHeaders = Object.keys(event.headers).filter(
    (h) =>
...
async function simulateTokenRefresh(refreshToken, event = {}) {
  ...
  if (
    ...
    global.usedTokens?.has(refreshToken)
  ) {
...
async function hashPassword(password) {
  ...
  return `$2a$10$${Buffer.from(password + "salt")
    .toString("base64")
    .substr(0, 22)}`;
}
