...
describe('API Security Tests', () => {
  ...
  describe('CORS Security', () => {
    ...
    it('should restrict allowed headers', async () => {
      ...
      for (const header of unauthorizedHeaders) {
        ...
        expect(allowedHeaders.toLowerCase()).not.toContain(header.toLowerCase());
      }
    });
  });
  ...
  describe('Rate Limiting', () => {
    ...
    it('should have different rate limits for different endpoints', async () => {
      ...
      const adminRequests = [];
      for (let i = 0; i < 20; i++) {
        adminRequests.push(simulateRateLimitedRequest(clientIP, '/admin/users'));
      }
      ...
      for (let i = 0; i < 20; i++) {
        regularRequests.push(simulateRateLimitedRequest(clientIP, '/templates'));
      }
      
      const regularResponses = await Promise.all(regularRequests);
      const regularThrottled = regularResponses.filter(r => r.statusCode === 429);
    ...
    it('should implement progressive rate limiting', async () => {
      ...
      for (let i = 1; i < responseTimes.length; i++) {
        if (responseTimes[i - 1] < 1000) { // Skip if already at max delay
    ...
    it('should rate limit by user ID for authenticated requests', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        requests.push(simulateAuthenticatedRateLimitedRequest(userId, '/prompts'));
      }
  ...
  describe('Response Security', () => {
    ...
    it('should sanitize response data', async () => {
      ...
      const response = await simulateResponseSanitization(event, maliciousTemplate);
  ...
  describe('Timing Attack Prevention', () => {
    ...
    it('should prevent username enumeration through timing', async () => {
      ...
      const existingTime = await measureLoginTime(existingEmail, 'wrong-password');
...
async function simulateCorsRequest(event) {
  ...
  if (requestHeaders) {
    ...
    if (validHeaders.length > 0) {
      response.headers['Access-Control-Allow-Headers'] = validHeaders.join(', ');
    }
  }
...
async function simulateUrlLengthLimit(event) {
  const url = event.path + (event.rawQueryString ? '?' + event.rawQueryString : '');
...
async function simulateDangerousHeaders(event) {
  ...
  if (event.headers?.['Host'] && !event.headers['Host'].includes('gravyprompts.com')) {
