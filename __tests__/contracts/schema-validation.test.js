...
const schemas = {
  template: {
    ...
    required: ['templateId', 'title', 'content', 'tags', 'visibility', 'status', 'authorId', 'authorEmail', 'createdAt'],
  ...
  userPrompt: {
    ...
    required: ['promptId', 'userId', 'templateId', 'title', 'content', 'variables', 'createdAt'],
    additionalProperties: false
  },
...
describe('Contract & Schema Validation Tests', () => {
  ...
  describe('Database Schema Validation', () => {
    it('should validate template database records', () => {
      const validTemplate = {
        ...
        content: 'Dear {{customerName}}, thank you for choosing {{companyName}}!',
  ...
  describe('Event Schema Validation', () => {
    ...
    it('should reject invalid event structures', () => {
      ...
      invalidEvents.forEach(event => {
        ...
        if (eventType && eventValidators[`template${eventType.charAt(0).toUpperCase()}${eventType.slice(1)}`]) {
          const validator = eventValidators[`template${eventType.charAt(0).toUpperCase()}${eventType.slice(1)}`];
  ...
  describe('Data Migration Compatibility', () => {
        ...
        properties: {
          ...
          up: {
            ...
            items: {
              type: 'object',
              properties: {
                operation: { type: 'string', enum: ['add_field', 'remove_field', 'rename_field', 'update_index'] },
                table: { type: 'string' },
                field: { type: 'string' },
                details: { type: 'object' }
              },
    ...
    it('should validate schema versioning', () => {
      const schemaVersions = [
        ...
        { version: '1.1.0', schemas: { template: schemas.template, userPrompt: schemas.userPrompt } }
      ];
    ...
    it('should validate third-party API contracts', () => {
      ...
      const cognitoUserSchema = {
        ...
        properties: {
          ...
          UserStatus: { type: 'string', enum: ['CONFIRMED', 'UNCONFIRMED', 'ARCHIVED'] }
        },
  ...
  describe('Schema Evolution Testing', () => {
    it('should support additive schema changes', () => {
      ...
      const newData = { id: '123', name: 'Test', description: 'Test description' };
