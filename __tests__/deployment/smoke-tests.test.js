...
describe('Deployment Smoke Tests', () => {
  ...
  describe('Environment Verification', () => {
    ...
    it('should have correct CORS configuration', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.headers['access-control-allow-origin']).toBe(config.webUrl);
  ...
  describe('Infrastructure Health Checks', () => {
    ...
    it('should have accessible static assets', async () => {
      // Test CSS loading
      const cssResponse = await axios.get(`${config.webUrl}/_next/static/css/app.css`, {
        timeout: 5000,
      ...
      const jsResponse = await axios.get(`${config.webUrl}/_next/static/chunks/main.js`, {
        timeout: 5000,
  ...
  describe('Critical Path Functionality', () => {
    ...
    it('should handle template search', async () => {
      const response = await axios.get(`${config.apiUrl}/templates?search=email&limit=3`, {
        timeout: 10000
      });
    ...
    it('should return 404 for non-existent resources', async () => {
      const response = await axios.get(`${config.apiUrl}/templates/non-existent-template`, {
        timeout: 5000,
        validateStatus: () => true
      });
  ...
  describe('Error Handling Validation', () => {
    it('should return proper error responses', async () => {
      ...
      const response = await axios.post(`${config.apiUrl}/templates`, {
    ...
    it('should handle database connection errors gracefully', async () => {
      ...
      const response = await axios.get(`${config.apiUrl}/templates/load-test-very-heavy-operation`, {
        timeout: 30000,
        validateStatus: () => true
      });
  ...
  describe('Data Consistency Validation', () => {
    ...
    it('should maintain data integrity across services', async () => {
      ...
      const listResponse = await axios.get(`${config.apiUrl}/templates?limit=1`, {
        timeout: 5000
      });
  ...
  describe('Monitoring and Logging Validation', () => {
    it('should generate proper access logs', async () => {
      ...
      const response = await axios.get(`${config.apiUrl}/templates?test=smoke-test`, {
        headers: {
  ...
  describe('Rollback Validation', () => {
    it('should maintain API compatibility during rollbacks', async () => {
      ...
      const response = await axios.post(`${config.apiUrl}/templates`, legacyRequest, {
        headers: {
          'Content-Type': 'application/json',
          'API-Version': 'v1'
        },
