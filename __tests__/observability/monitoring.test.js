...
describe("Monitoring & Observability Tests", () => {
  ...
  describe("Metrics Collection Tests", () => {
    it("should collect application performance metrics", () => {
      ...
      const counterMetric = global.metricData.find(
        (m) => m.name === "api.requests.total",
      );
      expect(counterMetric.value).toBe(1);
      expect(counterMetric.tags.method).toBe("GET");
      expect(counterMetric.tags.status).toBe("200");

      const timerMetric = global.metricData.find(
        (m) => m.name === "api.request.duration",
      );
      expect(timerMetric.value).toBe(150);

      const gaugeMetric = global.metricData.find(
        (m) => m.name === "api.active_connections",
      );
    ...
    it("should collect business metrics", () => {
      ...
      metrics.incrementCounter("templates.viewed", {
        templateId: "template-123",
      });
      ...
      const createdMetric = global.metricData.find(
        (m) => m.name === "templates.created",
      );
      expect(createdMetric.value).toBe(1);

      const totalCountMetric = global.metricData.find(
        (m) => m.name === "templates.total_count",
      );
    ...
    it("should collect infrastructure metrics", () => {
      ...
      // Lambda metrics
      metrics.recordGauge("lambda.duration", 2500, {
        function: "template-list",
      });
      metrics.recordGauge("lambda.memory_used", 128, {
        function: "template-list",
      });
      metrics.incrementCounter("lambda.cold_starts", {
        function: "template-list",
      });

      // DynamoDB metrics
      metrics.recordTimer("dynamodb.query.duration", 45, {
        table: "templates",
      });
      metrics.incrementCounter("dynamodb.throttled_requests", {
        table: "templates",
      });
      metrics.recordGauge("dynamodb.consumed_read_capacity", 15.5, {
        table: "templates",
      });

      expect(global.metricData).toHaveLength(6);

      const lambdaDuration = global.metricData.find(
        (m) => m.name === "lambda.duration",
      );
      expect(lambdaDuration.value).toBe(2500);
      expect(lambdaDuration.tags.function).toBe("template-list");

      const dbCapacity = global.metricData.find(
        (m) => m.name === "dynamodb.consumed_read_capacity",
      );
    ...
    it("should handle metric collection errors gracefully", () => {
      ...
      mockCloudWatch.putMetricData.mockRejectedValueOnce(
  ...
  describe("Alerting Tests", () => {
    it("should trigger alerts for high error rates", () => {
      ...
      expect(alert.metadata.threshold).toBe(0.1);
    });
    ...
    it("should prevent alert flooding with rate limiting", () => {
      ...
      for (let i = 0; i < 5; i++) {
        alertManager.checkErrorRate(0.15, 0.1);
      }

      // Should only send 3 alerts within the rate limit window
      expect(
        global.sentAlerts.filter((a) => a.type === "error_rate"),
      ).toHaveLength(3);
    });
...
function createMockMetrics(config = {}) {
  ...
  return {
    ...
    recordTimer: (name, duration, tags = {}) =>
      addMetric(name, duration, tags, "timer"),
    recordGauge: (name, value, tags = {}) =>
      addMetric(name, value, tags, "gauge"),
    flush: submitBatch,
  };
}
...
function createMockHealthChecker() {
  return {
    async getHealth() {
      ...
      const allHealthy = Object.values(checks).every(
        (check) =>
          ...
          (check.status !== "unhealthy" &&
            Object.values(check).every((s) => s.status === "healthy")),
      );
      ...
      const anyUnhealthy = Object.values(checks).some(
        (check) =>
          check.status === "unhealthy" ||
          (check.status !== "healthy" &&
            Object.values(check).some((s) => s.status === "unhealthy")),
      );
    ...
    async checkDatabase() {
      const dbCheck = global.healthChecks.database || {
        status: "healthy",
        responseTime: 25,
      };
      return {
    ...
    async checkExternalServices() {
      return {
        cognito: global.healthChecks.cognito || {
          status: "healthy",
          responseTime: 45,
        },
        s3: global.healthChecks.s3 || { status: "healthy", responseTime: 60 },
        cloudwatch: global.healthChecks.cloudwatch || {
          status: "healthy",
          responseTime: 35,
        },
      };
...
function createMockAlertManager(config = {}) {
  ...
  function shouldSendAlert(alertType) {
    ...
    // Clean old alerts
    global.sentAlerts = global.sentAlerts.filter(
      (alert) => alert.timestamp > windowStart,
    );

    const recentAlerts = global.sentAlerts.filter(
