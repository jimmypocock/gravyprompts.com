...
const mockGetUsersWithPermissions =
  getUsersWithPermissions as jest.MockedFunction<
    typeof getUsersWithPermissions
  >;
const mockGrantPermission = grantPermission as jest.MockedFunction<
  typeof grantPermission
>;
const mockRevokePermission = revokePermission as jest.MockedFunction<
  typeof revokePermission
>;
...
describe("PermissionsManager", () => {
  ...
  it("should display empty state when no users have permissions", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No users with permissions yet"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should show/hide the add permission form", async () => {
    ...
    expect(
      screen.queryByPlaceholderText(/123e4567-e89b-12d3/i),
    ...
    expect(
      screen.getByPlaceholderText(/123e4567-e89b-12d3/i),
    ).toBeInTheDocument();
  ...
  it("should revoke permission with confirmation", async () => {
    ...
    const firstRevokeButton = revokeButtons.find((btn) =>
  ...
  it("should handle grant permission errors gracefully", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: /Grant Permission/i })[1],
    );
    ...
    expect(screen.getByPlaceholderText(/123e4567-e89b-12d3/i)).toHaveValue(
      "new-user",
    );
  });
  ...
  it("should display permission descriptions", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Can approve\/reject templates/),
      ).toBeInTheDocument();
    });
  });
