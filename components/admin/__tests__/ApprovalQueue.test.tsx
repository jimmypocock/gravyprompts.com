...
const mockGetApprovalQueue = getApprovalQueue as jest.MockedFunction<
  typeof getApprovalQueue
>;
const mockProcessApproval = processApproval as jest.MockedFunction<
  typeof processApproval
>;
...
describe("ApprovalQueue", () => {
  ...
  it("should load and display pending templates", async () => {
    ...
    expect(
      screen.getByText("By author1@test.com • 2 hours ago"),
    ).toBeInTheDocument();
  });
  ...
  it("should display empty state when no templates", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No templates pending approval"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should open preview modal when clicking preview button", async () => {
    ...
    expect(
      screen.getByText("Sample email template content"),
    ).toBeInTheDocument();
  ...
  it("should approve template successfully", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText("Email Marketing Template"),
  ...
  it("should handle rejection with reason", async () => {
    ...
    const reasonTextarea = screen.getByPlaceholderText(
      /explain why this template is being rejected/i,
    );
    await user.type(reasonTextarea, "Contains inappropriate content");

    // Add optional notes
    const notesTextarea = screen.getByPlaceholderText(
      /add any notes about this template/i,
    );
