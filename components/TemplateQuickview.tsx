...
export default function TemplateQuickview({ template, isOpen, onClose, onSavePrompt }: TemplateQuickviewProps) {
  const { user } = useAuth();
  ...
  const allVariablesFilled = template?.variables?.every(
  ...
  const handlePopulate = async () => {
    ...
    try {
      ...
      const populatedHtml = await editorRef.current.generatePopulatedContent(variableInputs);
              ...
              <div>
                {template.variables && template.variables.length > 0 ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 text-center">
                    <p className="text-gray-600">This template has no variables to fill in.</p>
