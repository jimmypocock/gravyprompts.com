...
describe("GoogleAnalytics", () => {
  ...
  beforeEach(() => {
    ...
    mockScript.mockImplementation(({ children, ...props }) => {
      if (props.id) {
        return (
          <script data-testid={props.id} {...props}>
            {children}
          </script>
        );
      }
  ...
  describe("Script component props", () => {
    ...
    it("should not include any additional props besides required ones", () => {
      ...
      expect(Object.keys(gtagScriptProps).sort()).toEqual(
        ["src", "strategy"].sort(),
      );

      // Second script should only have id, strategy, and children
      expect(Object.keys(inlineScriptProps).sort()).toEqual(
        ["id", "strategy", "children"].sort(),
      );
    });
  });
