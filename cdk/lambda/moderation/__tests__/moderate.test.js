...
describe("Moderation Lambda", () => {
  ...
  describe("Content moderation logic", () => {
    ...
    it("should flag excessive capitalization", async () => {
      ...
      expect(updateCall.ExpressionAttributeValues[":details"].reason).toBe(
        ...
        Records: [
          {
            ...
            dynamodb: {
              NewImage: {
                ...
                content: {
                  S: "spam spam spam spam spam spam spam spam spam spam spam",
                },
                visibility: { S: "public" },
              },
            },
  ...
  describe("Duplicate prevention", () => {
    ...
    it("should handle conditional check failures", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        "Template template-123 was already processed",
      );
    });
  });
  ...
  describe("Content hash", () => {
    it("should create consistent content hash", async () => {
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[0][0];
      expect(
        updateCall.ExpressionAttributeValues[":contentHash"],
      ).toBeDefined();
      expect(updateCall.ExpressionAttributeValues[":contentHash"]).toHaveLength(
        32,
      ); // MD5 hash length
    });
  });
