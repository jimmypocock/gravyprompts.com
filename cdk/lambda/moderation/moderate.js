...
const { docClient, stripHtml } = require("/opt/nodejs/utils");
  ...
  for (const record of event.Records) {
    try {
      ...
      if (record.eventName === "INSERT" || record.eventName === "MODIFY") {
        ...
        const isModificationUpdate =
          record.eventName === "MODIFY" &&
          newImage.moderatedAt?.S &&
          (!oldImage?.moderatedAt?.S ||
            newImage.moderatedAt.S !== oldImage.moderatedAt.S);

        if (isModificationUpdate) {
          console.log(
            `Skipping template ${templateId} - this is a moderation update`,
          );
          continue;
        }
        ...
        if (currentModerationStatus && currentModerationStatus !== "pending") {
          console.log(
            `Skipping template ${templateId} - already has moderation status: ${currentModerationStatus}`,
          );
          continue;
        }
        ...
        try {
          ...
          await docClient.send(
            new UpdateCommand({
              ...
              Key: { templateId },
              UpdateExpression:
                "SET moderationStatus = :status, moderationDetails = :details, moderatedAt = :moderatedAt, #contentHash = :contentHash",
          ...
          console.log(
            `Successfully moderated template ${templateId}: ${moderationResult.status}`,
          );
        } catch (error) {
