...
describe('Share Template Lambda', () => {
  ...
  describe('Generate share link', () => {
    it('should generate share link successfully', async () => {
      ...
      expect(parsedResponse.shareUrl).toBe('https://test.gravyprompts.com/templates/template-123?token=share-token-123');
      ...
      expect(updateCall.ExpressionAttributeValues[':shareTokens']).toHaveProperty('share-token-123');
      // Should not include the expired token
      expect(updateCall.ExpressionAttributeValues[':shareTokens']).not.toHaveProperty('old-token');
    });
    ...
    it('should clean up expired tokens when generating new one', async () => {
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[1][0];
      const updatedTokens = updateCall.ExpressionAttributeValues[':shareTokens'];
  ...
  describe('Add viewers', () => {
    ...
    it('should return error if no valid users found', async () => {
      ...
      expect(parsedResponse.error).toBe('No valid users found with the provided emails');
    });
    ...
    it('should not add duplicate viewers', async () => {
      ...
      mockCognitoClient.send.mockResolvedValueOnce({
        UserAttributes: [
          { Name: 'sub', Value: 'existing-viewer' }
        ]
      });
  ...
  describe('Authorization and validation', () => {
    ...
    it('should validate action parameter', async () => {
      ...
      expect(response.statusCode).toBe(400);
      expect(parsedResponse.error).toBe('Invalid action. Must be one of: add, remove, generate_link');
    });
    ...
    it('should reject sharing non-owned templates', async () => {
      ...
      expect(parsedResponse.error).toBe('You can only share your own templates');
    });
  ...
  describe('Rate limiting', () => {
    ...
    it('should check rate limit with custom limits', async () => {
      ...
      expect(mockUtils.checkRateLimit).toHaveBeenCalledWith('user-123', 'share_template', {
        perMinute: 10,
        perHour: 100
      });
    });
