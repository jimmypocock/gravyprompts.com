...
describe('Health Check Lambda', () => {
  ...
  describe('Response format', () => {
    ...
    it('should always return 200 regardless of event content', async () => {
      ...
      const events = [
        ...
        createMockEvent({ httpMethod: 'POST', path: '/health', body: '{"test": true}' }),
        ...
        createMockEvent({ httpMethod: 'GET', path: '/health', queryStringParameters: { foo: 'bar' } }),
        createMockEvent({ httpMethod: 'GET', path: '/health', headers: { 'X-Custom': 'header' } }),
        {} // Empty event
      ];
  ...
  describe('Module caching', () => {
    it('should work correctly when module is cached', async () => {
      ...
      const response1 = await handler1(createMockEvent({ httpMethod: 'GET', path: '/health' }));
      
      // Second call (module should be cached)
      const { handler: handler2 } = require('../health');
      const response2 = await handler2(createMockEvent({ httpMethod: 'GET', path: '/health' }));
