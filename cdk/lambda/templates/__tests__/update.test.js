...
describe('Update Template Lambda', () => {
  ...
  describe('Successful updates', () => {
    it('should update template title successfully', async () => {
      ...
      expect(updateCall.ExpressionAttributeValues[':title']).toBe('Updated Title');
    });
    ...
    it('should update template content and extract variables', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.template.variables).toEqual(['firstName', 'lastName']);
      
      // Verify content was sanitized and variables extracted
      expect(mockUtils.sanitizeHtml).toHaveBeenCalledWith('Hello {{firstName}} {{lastName}}!');
      expect(mockUtils.extractVariables).toHaveBeenCalledWith('Hello {{firstName}} {{lastName}}!');
    });

    it('should update visibility from private to public and reset moderation', async () => {
      const privateTemplate = { ...existingTemplate, visibility: 'private', moderationStatus: 'not_required' };
      const user = createMockUser({ sub: 'user-123' });
      getUserFromEvent.mockResolvedValue(user);
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[1][0];
      expect(updateCall.ExpressionAttributeValues[':moderationStatus']).toBe('pending');
      expect(updateCall.ExpressionAttributeValues[':moderationDetails']).toBeNull();
    });
  ...
  describe('Validation', () => {
    it('should reject invalid visibility value', async () => {
      ...
      expect(parsedResponse.error).toBe('Visibility must be either public or private');
    });
    ...
    it('should reject updates that fail template validation', async () => {
      ...
      mockUtils.validateTemplate.mockReturnValue(['Content is too long', 'Title contains invalid characters']);
      ...
      expect(parsedResponse.details).toEqual(['Content is too long', 'Title contains invalid characters']);
    });
    ...
    it('should trim and lowercase tags', async () => {
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[1][0];
      expect(updateCall.ExpressionAttributeValues[':tags']).toEqual(['greeting', 'welcome', 'test']);
    });
    ...
    it('should trim title', async () => {
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[1][0];
      expect(updateCall.ExpressionAttributeValues[':title']).toBe('Trimmed Title');
    });
  });
  ...
  describe('Edge cases', () => {
    it('should not change visibility from public to public', async () => {
      ...
      const updateCall = mockDocClient.mockSend.mock.calls[1][0];
      expect(updateCall.ExpressionAttributeValues[':moderationStatus']).toBeUndefined();
    });
