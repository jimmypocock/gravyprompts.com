...
describe('Populate Template Lambda', () => {
  ...
  const htmlTemplate = {
    ...
    content: '<h1>Hello [[name]]!</h1><p>Welcome to <strong>[[company]]</strong></p>'
  };
  ...
  describe('Successful population', () => {
    it('should populate template with all variables', async () => {
      ...
      expect(parsedResponse.populatedContent).toBe('Hello John Doe, Welcome to Acme Corp! Your email is john@acme.com.');
    ...
    it('should populate with partial variables and warn about missing ones', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Hello John Doe, Welcome to Acme Corp! Your email is [[email]].');
      expect(parsedResponse.variables.missing).toEqual(['email']);
      expect(parsedResponse.warning).toBe('Missing values for variables: email');
    });
    ...
    it('should return plain text when returnHtml is false', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Hello John Doe!Welcome to Acme Corp');
    });

    it('should sanitize variable values to prevent XSS', async () => {
      mockUtils.getUserIdFromEvent.mockReturnValue('user-456');
      mockUtils.sanitizeHtml.mockImplementation(val => val.replace(/<script>/g, '&lt;script&gt;'));
      ...
      expect(mockUtils.sanitizeHtml).toHaveBeenCalledWith('<script>alert("XSS")</script>');
    });
    ...
    it('should handle numeric and boolean variable values', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Your order #12345 totals $99.99');
    });
  });
  ...
  describe('Access control', () => {
    ...
    it('should deny access to private template for non-viewer', async () => {
      ...
      expect(parsedResponse.error).toBe('You do not have access to this template');
    });
    ...
    it('should deny access with expired share token', async () => {
      ...
      mockDocClient.mockSend.mockResolvedValueOnce({ Item: templateWithExpiredToken });
  ...
  describe('Usage tracking', () => {
    ...
    it('should continue even if tracking fails', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Hello John, Welcome to Acme! Your email is john@acme.com.');
      
      // Should log the error
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error tracking usage:', expect.any(Error));
  ...
  describe('Edge cases', () => {
    it('should handle null and undefined variable values', async () => {
      ...
      expect(parsedResponse.warning).toBe('Missing values for variables: name, company');
    });
    ...
    it('should handle templates with no variables', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('This is a static template with no variables.');
    ...
    it('should handle variables with special regex characters', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Price: $99.99 (includes 10% ($9.99))');
    });
    ...
    it('should handle repeated variable placeholders', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(parsedResponse.populatedContent).toBe('Dear John, John has won! Congratulations John!');
    });
  });
});