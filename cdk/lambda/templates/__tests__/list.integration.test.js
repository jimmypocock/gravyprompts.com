...
describe('Templates List Handler - Real Integration Tests', () => {
  ...
  describe('Search Functionality - What the UI Actually Uses', () => {
    ...
    it('should find templates with partial title match', async () => {
      ...
      const emailMatches = body.items.filter(template => {
        const hasEmailMatch = 
          ...
          (template.preview && template.preview.toLowerCase().includes('email'));
    ...
    it('should search within template content', async () => {
      ...
      const hasContentMatch = body.items.some(t => 
        t.preview && t.preview.toLowerCase().includes('target audience')
      );
    ...
    it('should handle multi-term search', async () => {
      ...
      const hasMarketing = topResult.title.toLowerCase().includes('marketing') ||
                          (topResult.preview && topResult.preview.toLowerCase().includes('marketing'));
      const hasStrategy = topResult.title.toLowerCase().includes('strategy') ||
                         (topResult.preview && topResult.preview.toLowerCase().includes('strategy'));
    ...
    it('should boost popular templates in search results', async () => {
      ...
      const coldOutreachIndex = body.items.findIndex(t => 
        t.title === 'Cold Outreach Email Template'
      );
    ...
    it('should respect visibility filters', async () => {
      ...
      const privateTemplate = body.items.find(t => 
        t.title === 'User Testing Feedback Request'
      );
    ...
    it('should filter by moderation status', async () => {
      ...
      const pendingTemplate = body.items.find(t => 
        t.title === 'Email Newsletter Template'
      );
  ...
  describe('Filter Options - UI Filter Dropdown', () => {
    ...
    it('should return templates sorted by popularity for "popular" filter', async () => {
      ...
      for (let i = 1; i < body.items.length; i++) {
        expect(body.items[i-1].useCount).toBeGreaterThanOrEqual(body.items[i].useCount);
      }
    });
...
describe('Search Scoring Algorithm - Detailed Tests', () => {
  ...
  it('should handle variable name searches', async () => {
    ...
    const hasVariable = body.items.some(t => 
      t.variables && t.variables.includes('cta_text')
    );
