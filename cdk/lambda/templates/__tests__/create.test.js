...
describe('Create Template Lambda', () => {
  ...
  describe('Successful template creation', () => {
    it('should create a public template successfully', async () => {
      const user = createMockUser({ sub: 'user-123', email: 'test@example.com' });
  ...
  describe('Validation failures', () => {
    it('should reject template with validation errors', async () => {
      ...
      mockUtils.validateTemplate.mockReturnValue(['Title is required', 'Content is too short']);
      ...
      expect(parsedResponse.details).toEqual(['Title is required', 'Content is too short']);
    });
  });
  ...
  describe('Variable extraction', () => {
    it('should extract and store template variables', async () => {
      ...
      mockUtils.extractVariables.mockReturnValue(['firstName', 'lastName', 'email']);
      ...
      const event = createMockEvent({
        ...
        body: JSON.stringify({
          ...
          content: 'Hello {{firstName}} {{lastName}}, we will contact you at {{email}}',
          tags: ['contact'],
          visibility: 'public'
        })
      });
      ...
      expect(response.statusCode).toBe(201);
      expect(parsedResponse.template.variables).toEqual(['firstName', 'lastName', 'email']);
      
      // Verify that extractVariables was called with the content
      expect(mockUtils.extractVariables).toHaveBeenCalledWith('Hello {{firstName}} {{lastName}}, we will contact you at {{email}}');
    });
  });
});