...
export class MonitoringStackAmplify extends Stack {
  ...
  constructor(scope: Construct, id: string, props: MonitoringStackAmplifyProps) {
    ...
    // Add alarm actions
    billingAlarm10.addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));
    billingAlarm50.addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));
    billingAlarm100.addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));
    ...
    if (props.apiGatewayName) {
      ...
      apiErrorAlarm = new cloudwatch.Alarm(this, 'Api4xxAlarm', {
        ...
        evaluationPeriods: 1,
        treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
        alarmDescription: 'Alert when API has more than 10 4XX errors in 5 minutes',
      });
