    ...
    for (const item of scanResult.Items) {
      ...
      if (key) {
        ...
        if (deletedCount % 10 === 0) {
          process.stdout.write(`   Deleted ${deletedCount}/${scanResult.Items.length} items\r`);
        }
      }
    }
...
async function checkDynamoDB() {
  try {
    ...
    if (error.name === 'ResourceNotFoundException') {
      console.error('❌ Table "local-templates" not found. Make sure local DynamoDB is running with tables created.');
    } else {
      console.error('❌ Cannot connect to local DynamoDB. Make sure it\'s running on port 8000.');
    }
    return false;
  }
}
