...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (
      ...
      (
        window as Window & { getAuthDebugInfo?: () => Promise<unknown> }
      ).getAuthDebugInfo = async () => {
        try {
        ...
        try {
          ...
          setError(
            (retryErr as Error).message ||
              "Failed to sign in after clearing session",
          );
          throw retryErr;
        }
      }
  ...
  const updateProfile = async (updates: Partial<UserProfile>) => {
    try {
      ...
      if (updates.fullname !== undefined)
        attributeUpdates.name = updates.fullname;
      if (updates.bio !== undefined)
        attributeUpdates["custom:bio"] = updates.bio;
      if (updates.github !== undefined)
        attributeUpdates["custom:github"] = updates.github;
      if (updates.twitter !== undefined)
        attributeUpdates["custom:twitter"] = updates.twitter;
      if (updates.linkedin !== undefined)
        attributeUpdates["custom:linkedin"] = updates.linkedin;
  ...
  const getIdToken = async (): Promise<string> => {
    try {
      ...
      if (
        ...
        const cachedToken =
          sessionCacheRef.current.session.tokens?.idToken?.toString();
        if (cachedToken) {
          return cachedToken;
        }
      }
